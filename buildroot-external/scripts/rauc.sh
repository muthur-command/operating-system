...
function prepare_rauc_signing() {
    ...
    if [ ! -f "${key}" ]; then
        ...
        "${BR2_EXTERNAL_MCOS_PATH}"/scripts/generate-signing-key.sh "${cert}" "${key}"
    fi
}
...
function write_rauc_config() {
    ...
    (
        "${HOST_DIR}/bin/tempio" \
            -template "${BR2_EXTERNAL_MCOS_PATH}/ota/system.conf.gtpl"
...
function install_rauc_certs() {
    ...
    if [ "${DEPLOYMENT}" == "development" ]; then
        ...
        cp "${BR2_EXTERNAL_MCOS_PATH}/ota/dev-ca.pem" "${TARGET_DIR}/etc/rauc/keyring.pem"
    else
        cp "${BR2_EXTERNAL_MCOS_PATH}/ota/rel-ca.pem" "${TARGET_DIR}/etc/rauc/keyring.pem"
    fi
    ...
    if ! openssl verify -CAfile "${BR2_EXTERNAL_MCOS_PATH}/ota/dev-ca.pem" -no-CApath "${cert}"; then
...
function install_bootloader_config() {
    if [ "${BOOTLOADER}" == "uboot" ]; then
        ...
        echo -e "/dev/disk/by-partlabel/mcos-bootstate\t0x0000\t${BOOT_ENV_SIZE}" > "${TARGET_DIR}/etc/fw_env.config"
    fi

    # Fix MBR
    if [ "${PARTITION_TABLE_TYPE}" == "mbr" ]; then
        mkdir -p "${TARGET_DIR}/usr/lib/udev/rules.d"
        cp -f "${BR2_EXTERNAL_MCOS_PATH}/bootloader/mbr-part.rules" "${TARGET_DIR}/usr/lib/udev/rules.d/"
    fi
}
